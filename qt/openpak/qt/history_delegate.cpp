...
namespace {
// Citron's PlayTime::ReadablePlayTime, kept local so the delegate has no host dependency.
QString ReadablePlayTime(u64 seconds) {
    const u64 hours = seconds / 3600;
    const u64 minutes = (seconds % 3600) / 60;
...
void NextendoHistoryDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    const QString sub = tr("%1 played \xC2\xB7 %2")
                            .arg(ReadablePlayTime(seconds), RelativeLastPlayed(last_played));
...
QColor NextendoHistoryDelegate::CardBg() const {
    return openpak::qt::Host::Current()->IsDarkTheme() ? QColor(32, 32, 36) : QColor(236, 236, 240);
}

QColor NextendoHistoryDelegate::FgColor() const {
    return openpak::qt::Host::Current()->IsDarkTheme() ? QColor(230, 230, 234) : QColor(22, 22, 28);
}

QColor NextendoHistoryDelegate::DimColor() const {
    return openpak::qt::Host::Current()->IsDarkTheme() ? QColor(145, 145, 155) : QColor(100, 100, 112);
}
